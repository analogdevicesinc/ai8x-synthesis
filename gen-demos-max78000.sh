...
./ai8xize.py -e --verbose --top-level cnn -L --test-dir $SDK_TARGET --prefix cats-dogs --checkpoint-file trained/ai85-catsdogs-chw.pth.tar --config-file networks/cats-dogs-chw.yaml --device MAX78000 --compact-data --mexpress --softmax --display-checkpoint --embedded-code $CLOCK_TRIM $@